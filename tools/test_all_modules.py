...
def run_pytest(
    test_file: Path, modulo_dir: Path, verbose: bool = False, quiet: bool = False
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        # Ejecutar pytest
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=300  # 5 minutos máximo por modulo
        )
...
def run_tests(
    ...
    stats = {"total": len(tests), "passed": 0, "failed": 0, "skipped": 0, "results": {}}
...
Ejemplos:
  ...
  python tools/test_all_modules.py --show-output
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Muestra output detallado de pytest"
    )

    parser.add_argument("--quiet", "-q", action="store_true", help="Muestra solo resumen mínimo")
    ...
    parser.add_argument(
        "--show-output", "-s", action="store_true", help="Muestra output de tests que fallan"
    )

    parser.add_argument(
        "--no-integration", action="store_true", help="No ejecuta tests de integración"
    )
    ...
    if not args.no_integration:
        integration_success = run_integration_tests(
            project_root, verbose=args.verbose, quiet=args.quiet
        )
